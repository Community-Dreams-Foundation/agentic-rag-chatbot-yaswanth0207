"""Validates sanity_output.json against the expected schema."""
        ...
        for i, item in enumerate(qa):
            ...
            for j, c in enumerate(citations):
                ...
                if not is_non_empty_str(c.get("locator")):
                    fail(f"qa[{i}].citations[{j}].locator missing/empty")
                if not is_non_empty_str(c.get("snippet")):
                    fail(f"qa[{i}].citations[{j}].snippet missing/empty")

    if "B" in feat_set:
        if not Path("USER_MEMORY.md").exists() or not Path("COMPANY_MEMORY.md").exists():
